...
class Player
  ...
  def get_oponent_level(monster)
    ...
    return monster.get_basic_value
  end
  
  def should_convert
    dice = Dice.instance
    is_six = dice.next_number
    #    is_six = 6
    return is_six == 6
  end
  ...
  def apply_bad_consequence(bad)
    ...
    n_levels = bad.some_levels
    decrement_levels(n_levels)
    pending_bad = bad.adjust_to_fit_treasure_lists(@visible_treasures,@hidden_treasures)
    puts pending_bad.to_s
    if @visible_treasures.empty? && pending_bad.some_visible_treasures != 0
      pending_bad.some_visible_treasures = 0
    else 
      if (bad.some_visible_treasures > @visible_treasures.size)
        pending_bad.some_visible_treasures = @visible_treasures.size
      else
        pending_bad.some_visible_treasures = bad.some_visible_treasures
      end
    end
    
    if @hidden_treasures.empty? && pending_bad.some_hidden_treasures != 0
      pending_bad.some_hidden_treasures = 0
    else 
      if (bad.some_hidden_treasures > @hidden_treasures.size)
        pending_bad.some_hidden_treasures = @hidden_treasures.size
      else
        pending_bad.some_hidden_treasures = bad.some_hidden_treasures
      end
    end
  ...
  def can_make_treasure_visible(treasure)
    ...
    if treasure.type == TreasureKind::BOTHHAND && count != 0
      return false
    end
  ...
  def combat(monster)
    ...
    monster_level = get_oponent_level(monster)
